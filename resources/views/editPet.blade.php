...
@section('content')
    <div class="container">
        ...
        <div class="adopt-form">
          <div class="adoptform">
            <p class="text-center fs-2 fw-bold"> Edit Pet </p>
            <form method="POST" action="/petgallery/update/{{$pets->id}}" enctype="multipart/form-data">
              @csrf
              <div class="row">
                <div class="col-md-12 ">
                  <label for="image" style="cursor: pointer;" class=" d-flex justify-content-center align-items-center">
                    <img id="preview-image-before-upload"
                         src="{{ asset('/img/'.$pets->image) }}"
                         alt="preview image"
                ...
                <div class="col-md-12 mt-5">
                  <input type="text" name="name" id="name" placeholder="Name" value="{{ $pets->name }}" required class="form-control" >
                     @error('name')
                ...
                <div class="col-md-12">
                  <select name="animal_type" id="animal_type" required class="form-select mb-2" >
                    <option value="{{ $pets->animal_type }}">{{ $pets->animal_type}}</option>
                  ...
                  <select name="gender" id="gender" required class="form-select mb-2">
                    ...
                    <option value="{{ $pets->gender }}">{{ $pets->gender }}</option>
                  ...
                  <input type="text" name="age" id="age" placeholder="Age" value="{{ $pets->age}}" required class="form-control">
                  @error('age')
                    <div class="text-red-500">{{ $message }}</div>
                  @enderror
                </div>  
                <div class="col-md-12">
                  <input type="text" name="color" id="color" placeholder="Pet Color" value="{{ $pets->color}}" required class="form-control mb-2">
                  @error('color')
                    <div class="text-red-500">{{ $message }}</div>
                  @enderror
                </div>    
                <div class="col-md-12">
                  <textarea name="description" id="description" rows="4" placeholder="Pet Description" class="form-control">{{ $pets->description}}</textarea>
                  @error('description')
                    <div class="text-red-500">{{ $message }}</div>
                  @enderror
                </div>        
              ...
              <div class="col-md-12 mt-2 text-center">
                <button type="submit" class="btn upload-btn">Save</button>
              </div>
            </div>
            </form>
          </div>
